...
function SidebarDesktop () {
    ...
    return (
        <>
            {isMobile && (
                <nav className="navbar navbar-light bg-light border-bottom d-md-none position-fixed top-0 start-0 w-100" style={{zIndex: 1060}}>
                    <div className="container-fluid d-flex align-items-center justify-content-between">
                        <button className="btn btn-light border" onClick={toggleSidebar} aria-label="Abrir menu lateral">
            ...
            {isMobile && sidebarOpen && (
                <div className="position-fixed top-0 start-0 w-100 h-100 d-md-none" style={{backgroundColor: 'rgba(0, 0, 0, 0.35)', zIndex: 1050, marginTop: '60px'}} onClick={toggleSidebar} aria-hidden="true"></div>
            )}
            <nav className="navbar flex-shrink-0 h-100 bg-light border-end d-md-block p-0" style={isMobile ? {position: 'fixed', top: '60px', left: 0, width: mobileWidth, minWidth: mobileWidth, height: 'calc(100vh - 60px)', transform: sidebarOpen ? 'translateX(0)' : 'translateX(-100%)', transition: 'transform 0.3s ease', zIndex: 1051} : {position: 'relative', width: isExpanded ? `${MAX_WIDTH}px` : `${MIN_WIDTH}px`, minWidth: isExpanded ? `${MAX_WIDTH}px` : `${MIN_WIDTH}px`, transition: 'all 0.3s ease'}} onTransitionEnd={handleTransition}>
                <ul className="navbar-nav d-flex flex-column h-100 w-100">
                    <li className={isExpanded ? "nav-item w-100" : "nav-item w-100 d-flex flex-column align-items-center"}>
                        <button className="btn w-100 py-0 d-none d-md-block" onClick={toggleSidebar}><i className={(isExpanded ? "bi bi-arrow-left" : "bi bi-arrow-right") + " fs-2"}></i></button>
                        ...
                        <ul className="navbar-nav collapse w-100 ps-4" id="collapse-alunos">
                            <li className="nav-item border-bottom w-100">
                                <Link to="/gerenciar-alunos" className="nav-link w-100">
                                    <i className={showText ? "bi bi-justify me-2" : "bi bi-justify"}></i>
                                    <span className={showText ? "d-inline" : "d-none"}>Gerenciar Alunos</span>
                                </Link>
                            </li>
                        </ul>
                    </li>
                    <li className={isExpanded ? "nav-item w-100" : "nav-item w-100 d-flex justify-content-center"}>
                        ...
                        <ul className="navbar-nav collapse w-100 ps-4" id="collapse-cursos">
                            <li className="nav-item border-bottom w-100">
                                <Link to="gerenciar-cursos" href="#" className="nav-link w-100">
                                    <i className={showText ? "bi bi-justify me-2" : "bi bi-justify"}></i>
                                    <span className={showText ? "d-inline" : "d-none"}>Gerenciar Cursos</span>
                                </Link>
                            </li>
                            <li className="nav-item border-bottom w-100">
                                <Link to="/gerenciar-turmas" className="nav-link w-100">
                                    <i className={showText ? "bi bi-justify me-2" : "bi bi-justify"}></i>
                                    <span className={showText ? "d-inline" : "d-none"}>Gerenciar Turmas</span>
                                </Link>
                            </li>
                        </ul>
                    </li>
